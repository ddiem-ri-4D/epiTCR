...
# Load predictions
test_files = [f"../../data/predToolsData/Imrex/test{i:02d}_pred.csv" for i in range(1, 16)]
...
for data in test_data:
    ...
    y_prob = data["prediction_score"].to_numpy()
...
roc_curves = [roc_curve(data["binder"], data["prediction_score"]) for data in test_data]
...
for i, (fpr, tpr, _) in enumerate(roc_curves):
    ...
    ax.plot(fpr, tpr, lw=1, alpha=0.3, label=f'Imrex - test{i+1:02d} (AUC = {aucs[i]:.3f})')
...
plt.savefig("../../analysis/figures/benchmarkImrexPretrained.png", dpi=600)
plt.savefig("../../analysis/figures/benchmarkImrexPretrained.pdf", dpi=600)
plt.show()
