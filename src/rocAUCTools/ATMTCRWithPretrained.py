...
# Load predictions
test_files = [f"../../data/predToolsData/ATMTCR/Pretrained/test{i:02d}_pred.csv" for i in range(1, 16)]
...
for i, data in enumerate(test_data):
    ...
    y_prob = data["predict_proba"].to_numpy()
...
roc_curves = [roc_curve(data["binder"], data["predict_proba"]) for data in test_data]
...
for i, (fpr, tpr, _) in enumerate(roc_curves):
    ...
    ax.plot(fpr, tpr, lw=1, alpha=0.3, label=f'ATM-TCR - test{i+1:02d} (AUC = {aucs[i]:.3f})')
